          ...
          <div class="card-body">
            <div class="d-flex align-items-start justify-content-between gap-3">
              ...
              <div class="text-end small text-muted">
                Submitted: <div class="fw-semibold">{{ $listing->created_at?->format('d M Y, h:i A') }}</div>
              </div>
            </div>
              ...
              <div class="row g-2">
                @foreach($listing->photos as $photo)
                  ...
                  <div class="col-6 col-md-4 col-lg-3">
                    ...
                    @if($photo->is_primary)
                      <div class="small mt-1"><span class="badge text-bg-primary">Primary</span></div>
                    @endif
                  </div>
                @endforeach
              </div>
          ...
          <div class="card-body">
            ...
            <div class="row g-3">
              <div class="col-md-6">
                ...
                <div class="text-muted small">
                  {{ $addr?->area ?? '' }}
                  @if($listing->city){{ $addr?->area ? ',' : '' }} {{ $listing->city->name }}@endif
                </div>
                @if($addr?->postcode)
                    ...
                    <table class="table table-sm mb-0">
                      ...
                      <tbody>
                        @foreach($listing->hours as $h)
                          <tr>
                            <td class="fw-semibold">{{ $h->day_name ?? $h->day ?? '—' }}</td>
                            <td>{{ $h->open_time ?? '—' }}</td>
                            <td>{{ $h->close_time ?? '—' }}</td>
                            <td>
      ...
      <div class="col-lg-4">
        <div class="card border-0 shadow-sm rounded-3 mb-3">
          <div class="card-body">
            ...
            <div class="mt-2 small">
              <div><span class="text-muted">Claimed:</span> <span class="fw-semibold">{{ $listing->is_claimed ? 'Yes' : 'No' }}</span></div>
            </div>
          </div>
        </div>
          ...
          <div class="card-body">
            ...
            @else
              <div class="small text-muted mb-2">
                Submitted at: <span class="fw-semibold">{{ $v->created_at?->format('d M Y, h:i A') ?? '—' }}</span>
              </div>
              ...
              <div class="d-grid gap-2">
                <div class="p-2 border rounded-3">
                  ...
                  @if($nidFront)
                    <a class="btn btn-sm btn-outline-primary w-100" target="_blank" rel="noopener"
                       href="{{ asset('storage/'.$nidFront) }}">
                      View Document
                    </a>
                  @else
