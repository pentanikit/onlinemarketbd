            ...
            <div class="card-body">
              <div class="d-flex align-items-start justify-content-between gap-3">
                <div>
                  ...
                  <div class="text-muted small">
                    ...
                    Current Status:
                    <span class="badge text-bg-{{ $listing->status === 'active' ? 'success' : ($listing->status === 'pending' ? 'warning' : 'secondary') }}">
                      {{ ucfirst($listing->status) }}
                    </span>
                  </div>
                </div>
                <div class="text-end small text-muted">
                  Created: <div class="fw-semibold">{{ $listing->created_at?->format('d M Y, h:i A') }}</div>
                </div>
              </div>
              ...
              <div class="row g-3">
                ...
                <div class="col-md-6">
                  ...
                  <input name="name" class="form-control" value="{{ old('name', $listing->name) }}" required>
                </div>

                <div class="col-md-6">
                  <label class="form-label">Slug (optional)</label>
                  <input name="slug" class="form-control" value="{{ old('slug', $listing->slug) }}">
                  <div class="form-text">If you change slug, it will auto-fix uniqueness.</div>
                </div>

                <div class="col-md-4">
                  <label class="form-label">Type</label>
                  <input name="type" class="form-control" value="{{ old('type', $listing->type) }}" placeholder="e.g. business, service">
                </div>

                <div class="col-md-4">
                  <label class="form-label">Price Level (0-5)</label>
                  <input type="number" min="0" max="5" name="price_level" class="form-control" value="{{ old('price_level', $listing->price_level) }}">
                </div>
                ...
                <div class="col-md-4">
                  ...
                  <select name="status" class="form-select" required>
                    @foreach(['pending','active','inactive','rejected'] as $s)
                      <option value="{{ $s }}" @selected(old('status', $listing->status)===$s)>{{ ucfirst($s) }}</option>
                    @endforeach
                  </select>
                </div>
                ...
                <div class="col-md-12">
                  ...
                  <input name="tagline" class="form-control" value="{{ old('tagline', $listing->tagline) }}">
                </div>
            ...
            <div class="card-body">
              ...
              <div class="row g-3">
                <div class="col-md-6">
                  ...
                  <input name="email" class="form-control" value="{{ old('email', $listing->email) }}">
                </div>
                <div class="col-md-6">
                  <label class="form-label">Phone</label>
                  <input name="phone" class="form-control" value="{{ old('phone', $listing->phone) }}">
                </div>
                <div class="col-md-6">
                  <label class="form-label">Website</label>
                  <input name="website" class="form-control" value="{{ old('website', $listing->website) }}" placeholder="https://">
                </div>
                <div class="col-md-3">
            ...
            <div class="card-body">
              ...
              <div class="row g-3">
                <div class="col-md-12">
                  ...
                  <input name="address_line" class="form-control" value="{{ old('address_line', $addr?->line1) }}">
                </div>
                <div class="col-md-6">
                  <label class="form-label">Area</label>
                  <input name="area" class="form-control" value="{{ old('area', $addr?->area) }}">
                </div>
                <div class="col-md-6">
                  <label class="form-label">Postcode</label>
                  <input name="postcode" class="form-control" value="{{ old('postcode', $addr?->postal_code) }}">
                </div>
              </div>
            </div>
          </div>
              ...
              @if($listing->photos->count())
                <div class="row g-2">
                  @foreach($listing->photos as $p)
                    ...
                    <div class="col-6">
                      <div class="border rounded-3 overflow-hidden bg-light" style="aspect-ratio: 4/3;">
                        @if($src)
                          <img src="{{ $src }}" alt="photo" style="width:100%;height:100%;object-fit:cover;">
                        @else
                      ...
                      <div class="mt-1 d-flex flex-column gap-1">
                        <label class="small">
                          <input type="radio" name="primary_photo" value="{{ $p->id }}"
                                 @checked(old('primary_photo', $listing->photos->firstWhere('is_primary',1)?->id) == $p->id)>
                          Primary
                        </label>
